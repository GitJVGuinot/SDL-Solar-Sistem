...
void Render::rotation(Vec3 rot)
{
  ...
  MathUtils::Orbit_Point(camera_, rot, target_);
...
SDL_Color renderColorLight(Vec3 point, Vec3 desp, Vec3 light, SDL_Color color)
{
  ...
  float euler = MathUtils::Radian_To_Euler(Vec3::Angle(point_vector, light_vector));

  if (euler < 0)
    euler *= -1;

  float angleRest = (euler * 255) / 180;
  float max = newColor[0];
...
void Render::renderThreadedPoints(Render_Figure &figure, bool renderLight)
{
  ...
  for (int i = 0; i < numThreads; i++)
  {
    ...
    // Handle any remaining points in the last threa
...
// These are proofs. But I need to keep it
////////////////////////////////////////////////////////////////////////////////////////////////////
  /*
  for(int i = 0; i < figure.n_points; i++)
  {
    if (threads.size() < current_threads_)
    {
      threads.push_back(std::thread(&Render::renderPoint, this, in_vert[i], figure.point[i], figure.desp, figure.light, figure.color, figure.model, figure.forceRender, renderLight));
    } else {
      threads[i % current_threads_].join();

      threads[i % current_threads_] = std::thread(&Render::renderPoint, this, in_vert[i], figure.point[i], figure.desp, figure.light, figure.color, figure.model, figure.forceRender, renderLight);
    }
  }
  ...
  */
  ...
  /*
  for(int i=0; i<figure.n_points; i+=current_threads_)
  {
    for(int j=0; j<current_threads_ && i+j<figure.n_points; j++)
    {
      threads.push_back(std::thread(&Render::renderPoint, this, std::ref(in_vert[i+j]), std::ref(figure.point[i+j]),
                                    std::ref(figure.desp), std::ref(figure.light), std::ref(figure.color),
                                    std::ref(figure.model), std::ref(figure.forceRender), renderLight));
    }
  ...
  */
  ...
  /*
  std::thread *threads_ = (std::thread*)calloc(current_threads_, sizeof(std::thread));
  for(int i=0; i<figure.n_points; i+=current_threads_)
  {

    for(int j=0; j<current_threads_; j++)
    {
      if( (i+j) < figure.n_points)
      {
        auto protoFunction = std::bind(&Render::renderPoint, &*this, in_vert[i+j], figure.point[i+j], figure.desp, figure.light, figure.color, figure.model, figure.forceRender, renderLight);
        threads_[j] = std::thread(protoFunction);
      }
    }
...
void Render::renderPoint(Render_Vert &ret_vert, Vec3 point, Vec3 desp, Vec3 light, SDL_Color color, Mat3 model, bool forceRender, bool renderLight)
{
  if (active(point) || forceRender)
  {
    ...
    Mat4 vMatrix = MathUtils::Mat4View(camera_, target_, up_);
...
void Render::cameraDraw(Keys *keys, SDL_Renderer *render, Vec2 max_win)
{
  ...
  Mat3 desp = Mat3::Translate(MathUtils::Vec3_Tr_Vec2(render_centers_));
  model = desp * scale;
