...
void Camera_Control(const char* str, Render &render, My_Window &win, MV::Pnt2 max_win){
  if (ImGui::Begin(str)){
    ...
    if(far!=render.getFar() || near!=render.getNear()) render.init(max_win, render.camara_, near, far);
...
void Planets_Control(const char* str, Esfera **planets, MV::Pnt3 **object_desp, int &max_planets, MV::Pnt2 max_win){
  ...
  if (ImGui::Begin(str)){
    ...
    for(int i = 0; i<max_planets; i++){
      ...
      desp.x*=-1;
      desp.z*=-1;
      if(desp.x!=0 || desp.y!=0 || desp.z!=0) localPlanets[i].translation(desp);
      ...
      rot.x*=-1;
      rot.y*=-1;
      if(rot.x!=0 || rot.y!=0 || rot.z!=0) localPlanets[i].rotation(rot);
      ...
      if(scale.x!=1 || scale.y!=1 || scale.z!=1) localPlanets[i].scale(scale);
