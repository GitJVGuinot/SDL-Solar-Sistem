...
{
  Vec3 extrem = MathUtils::TakeMin(points_, vertex_);
  float magn = extrem.Magnitude();

  scale_.x = scale_.y = scale_.z = dim_ = magn;

  float unitMagn = Vec3(1, 1, 1).Magnitude();

  if (magn > unitMagn || magn < unitMagn)
  {
    scale(unitMagn / magn);
  }
}
...
void Entity::draw(Keys *keys, SDL_Renderer *render, Render drawRender, Vec3 light, int id)
{
  ...
  if ((orbit_center_.x + orbit_center_.y + orbit_center_.z) == 0)
  {
    mov = Mat3::Translate({mov_.x, mov_.y});
  }
  else
  {
    mov = Mat3::Translate(MathUtils::Vec3_Tr_Vec2(orbit_center_));
  }
