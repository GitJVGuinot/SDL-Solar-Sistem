...
int main(int argc, char **argv)
{
  ...
  Esfera sun = *new Esfera(colores[iRand(MAX_COLORS-2, 0)], 10, {30,30,30}, {middle_win.x, middle_win.y, 0.0f});
  int max_planets = 4;
  ...
  *(planet + 3) = *new Esfera(colores[3], 3, {3, 3, 3}, {sun.desp_.x + 70, sun.desp_.y + 70, 0.0f}, {0, 0, 0}, {0.01f, -0.01f, 0.0f}, sun.desp_);
  ...
  while (win.runing)
  {
    ...
    Orbitar_Punto(mira, {fRand(0.75f,0), fRand(0.5f,0), fRand(0.25f,0)}, light);
